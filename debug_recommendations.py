...
def debug_recommendations():
    # Load algorithm and stations
    algo = HybridAlgorithm()
    with open('backend/data/charging_stations.json', 'r', encoding='utf-8') as f:
    ...
    for station in route_stations[:5]:  # Show first 5
        print(f"- {station['name']} (Detour: {station['detour']:.1f}km, Efficiency: {station['efficiency']:.3f})")
    ...
    if result['recommendations']:
        ...
        if scored_stations:
            ...
            print("\nTop 5 scored stations:")
            for i, station in enumerate(scored_stations[:5], 1):
