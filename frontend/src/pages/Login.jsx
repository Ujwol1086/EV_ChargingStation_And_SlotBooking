...
// Note: We request native geolocation post-login instead of a custom modal
...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        const target = result.is_admin ? "/admin" : "/dashboard";
        setPostLoginPath(target);
        // Always attempt native geolocation once post-login
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            ...
            (err) => {
              // Treat as maybe later: do not persist; proceed
              navigate(target);
            },
  ...
  // Removed custom modal handlers; using native geolocation prompt
        ...
        <div className="bg-gray-900/70 backdrop-blur-xl py-6 px-6 shadow-2xl rounded-2xl border border-gray-800/40">
          ...
          <div className="mt-4">
            <button
              ...
              className="w-full flex items-center justify-center px-4 py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-sm"
            >
              {isGoogleLoading ? (
                <div className="flex items-center">
